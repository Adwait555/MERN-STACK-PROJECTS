  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
        ...
        {user ? (
          <>
            <span style={{ color: '#fff', fontSize: 14 }}>{user.name} <span className="badge">{user.role}</span></span>
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="secondary" style={{ color: '#fff', borderColor: 'rgba(255,255,255,0.3)' }} onClick={() => { logout(); navigate('/login'); }}>Logout</motion.button>
          </>
        ) : (
          <>
